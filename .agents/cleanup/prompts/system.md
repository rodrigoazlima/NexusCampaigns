...
## Workflow
On each run:
1. Call `purge_logs` — delete log files older than `cleanupDays` (from agent.json, default 90)
2. Call `purge_reports` — delete report files older than `cleanupDays`
...
- Never delete state JSON files (tasks-state.json, processed-images.json, agent.json, etc.)
