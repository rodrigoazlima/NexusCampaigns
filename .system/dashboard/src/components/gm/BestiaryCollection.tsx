...
// ponytail: clone of NpcCollection specialized to the bestiary pillar (its types,
// guidance, pressure/flat/forced deltas). When a third pillar lands, lift the
// shared shell (filter bar, grid/table, ActionBtn, toast) into one CollectionView.
...
import { Tip } from './Tip'
import {
  Skull, Plus, Sparkles, ChevronDown, ChevronRight, Star, Loader2,
  Check, Flag, Archive, XCircle, ExternalLink, LayoutGrid, Table as TableIcon,
  AlertTriangle, Search,
} from 'lucide-react'

export type BestiaryItem = ReviewItem & {
  origin: 'draft' | 'canon'
...
const TYPE_CHIP = 'bg-red-500/10 text-red-400'
...
const TYPES = ['creature', 'monster', 'encounter'] as const
const PRESSURES = ['brute', 'swarm', 'social', 'puzzle'] as const
...
const isFlat = (i: BestiaryItem) => isEncounter(i) && !i.hasTerrain
const isForced = (i: BestiaryItem) => isEncounter(i) && !i.hasNonCombat
...
export default function BestiaryCollection({ items }: { items: BestiaryItem[] }) {
  ...
  const filtered = items
    ...
    .filter((i) => pressureFilter === 'all' || i.pressure === pressureFilter)
    ...
    .filter((i) => !flatOnly || isFlat(i))
    .filter((i) => !forcedOnly || isForced(i))
    .filter((i) =>
      !q ||
    ...
    .sort((a, b) => {
      // signature threats pin to the top regardless of sort (Delta 4)
  ...
  return (
    <div className="p-4 md:p-6">
                ...
                <>
                  ...
                  <div className="absolute right-0 mt-1 z-50 w-52 panel p-1 shadow-xl">
                    {TYPES.map((t) => (
                      ...
                      >
                        {t === 'encounter' ? 'Encounter (placed fight)' : `${t} (reusable stat entity)`}
                      </button>
                    ))}
                  </div>
                </>
      ...
      <div className="panel p-3 mb-4 flex flex-wrap gap-2 items-center">
        ...
        <select value={pressureFilter} onChange={(e) => setPressureFilter(e.target.value)} className={SELECT_CLS}>
        ...
        {([
          ...
          ['Flat', flatOnly, setFlatOnly],
          ['Forced', forcedOnly, setForcedOnly],
      ...
      {items.length === 0 ? (
        <div className="panel p-12 text-center">
          <Skull size={24} className="mx-auto text-zinc-600 mb-3" />
          <div className="text-zinc-400 text-sm">No creatures yet</div>
          <div className="flex items-center justify-center gap-3 mt-3">
            <button onClick={() => create('creature')} className="text-xs px-3 py-1.5 rounded bg-primary/15 text-primary border border-primary/30 hover:bg-primary/25 transition-colors">+ New creature</button>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
          {filtered.map((i) => {
            ...
            return (
              <div key={i.id} className="panel border border-surface-3 hover:border-red-500/30 transition-colors flex flex-col group overflow-hidden">
          ...
          <table className="w-full text-xs">
            <thead className="text-zinc-500 border-b border-surface-3">
              <tr className="text-left">
                ...
                <th className="p-2 font-medium">Pressure</th>
              ...
              {filtered.map((i) => {
                ...
                return (
                  <tr key={i.id} className="border-b border-surface-3/40 hover:bg-surface-2">
                    ...
                    <td className="p-2">
                      <Link href={cardHref(i)} className="font-mono text-zinc-200 hover:text-red-400 inline-flex items-center gap-1">
                        {i.id}
                        {i.signature && <Star size={10} className="text-yellow-400 fill-yellow-400" />}
                        {isFlat(i) && <span className="text-warning" title="flat fight">⚑</span>}
                        {isForced(i) && <span className="text-warning" title="forced fight">⚔</span>}
                      </Link>
                    </td>
