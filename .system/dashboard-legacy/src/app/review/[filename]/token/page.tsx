...
import { findMarkdownByIdentifier, VAULT_ROOT } from '@/lib/vault'
...
export default function TokenEditorPage({ params }: Props) {
  ...
  const sources: string[] = Array.isArray(fm.source) ? fm.source.map(String) : (fm.source ? [String(fm.source)] : [])
  ...
  const item: ReviewItem = {
    ...
    relationships: Array.isArray(fm.relationships) ? fm.relationships.map(String) : [],
    hasRelationships: Array.isArray(fm.relationships) && fm.relationships.length > 0,
    isOrphan: false,
  ...
  return <TokenEditor item={item} encoded={encoded} />
}
