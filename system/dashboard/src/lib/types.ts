...
export interface ReviewItem {
  ...
  filepath: string
  id: string
  // Stable identifier that survives slug/id renames. Empty only for legacy
  // items not yet backfilled.
  uuid: string
  type: string
  status: string
  quality: number
  ...
  // All other md drafts that share this item's source image (history / siblings).
  // Empty when the item has a unique source.
  history: DraftRef[]
}
...
export interface AgentConfig {
  cleanupDays?: number
  token_config?: TokenGeneratorConfig
...
// §0 setting frame, stored as a `type: campaign` doc in 03-Campaigns/.
