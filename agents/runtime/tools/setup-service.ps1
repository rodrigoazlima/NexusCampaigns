...
#
#   # Same, but make the vault its own git repo (only if it isn't one already):
#   pwsh -ExecutionPolicy Bypass -File setup-service.ps1 -VaultRoot "D:\Vaults\MyCampaign" -VaultGitInit
#
...
function Ensure-VaultGitRepo([string]$VaultPath) {
    if (Test-Path (Join-Path $VaultPath ".git")) {
        Log "Vault already a git repo: $VaultPath"
        return
    }

    Log "Initializing git repo for vault: $VaultPath"
    ...
    try {
        ...
        git add -A | Out-Null
        git commit -m "Initial vault import" --quiet | Out-Null
        Log "Vault git repo initialized and committed: $VaultPath"
    } catch {
        Log "Vault git init failed: $_" "ERROR"
    } finally {
        Pop-Location
    }
}
