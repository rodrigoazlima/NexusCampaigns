...
6. Harvests a free-form `candidate_tags` list from the LLM's own `visual_analysis` block (equipment/clothing/fantasy_features/environment_details arrays — see `_extract_candidate_tags` in `tools/classify_images.py`) and stores it on `processed-images.json` and the `inbox-queue.json` entry — never on note frontmatter. The classification agent reads it to seed its tag-library canonicalization; see `agents/classification/CLAUDE.md`.
