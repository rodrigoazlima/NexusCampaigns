...
`system/src/nexus/tasks/cleanup_agent.py` does three things each run:
...
python -m nexus.runner --task cleanup-agent --force

# Run the tool script directly
python -m nexus.tasks.cleanup_agent
...
- `system/src/nexus/runner.py (run: python -m nexus.runner)` is the only "static" orchestrator; every other `agents/<name>/` is a spec-driven agent
